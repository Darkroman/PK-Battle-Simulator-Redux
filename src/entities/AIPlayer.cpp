...
void AIPlayer::GetOpponentParty(Player* opponent)
{
...
void AIPlayer::InitEstimatedStatRanges(Player* target)
{
	int minIV{ 0 };
	int maxIV{ 31 };
	int minEV{ 0 };
...
void AIPlayer::PrecomputeRealStats(Player* target)
{
...
void AIPlayer::UpdateObservedMoves(BattlePokemon::pokemonMove* currentMove)
{
...
void AIPlayer::UpdateActivePokemon(BattlePokemon* activeOpponentMon)
{
    memory.activeOpponent.opponentActivePokemon = activeOpponentMon;
}

void AIPlayer::UpdateEnemyOffenseStats(BattleContext& context, BattlePokemon::pokemonMove* currentMove, BattlePokemon* source, BattlePokemon* target)
{
	...
	if (this->HasLightScreen() && !isCriticalHit && currentMove->GetCategoryEnum() == Category::Special)
	{
		observedDamage = (observedDamage * 2048) >> 10;
		multipliers.push_back(2048);
	}

	if (this->HasReflect() && !isCriticalHit && currentMove->GetCategoryEnum() == Category::Physical)
	{
		observedDamage = (observedDamage * 2048) >> 10;
		multipliers.push_back(2048);
	}
...
void AIPlayer::UpdateEnemyHPandDefenseStats(BattleContext& context, Player* targetPlayer, BattlePokemon::pokemonMove* currentMove, BattlePokemon* source, BattlePokemon* target)
{
	if (currentMove == nullptr)
	{
		return;
...
void AIPlayer::UpdateEnemySpeedStats(BattleContext& context, BattlePokemon::pokemonMove* sourceMove, BattlePokemon::pokemonMove* targetMove, BattlePokemon* source, BattlePokemon* target)
{
	if (sourceMove == nullptr || targetMove == nullptr)
	{
		return;
	}

	if (sourceMove->GetPriority() != targetMove->GetPriority())
	{
		return;
	}
...
std::array<PersistentMemory, 6>::iterator AIPlayer::FindActivePokemonSlot()
{
	auto it = std::find_if(
